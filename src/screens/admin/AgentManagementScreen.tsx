...
export const AgentManagementScreen: React.FC = () => {
  ...
  const loadAgents = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      Alert.alert('Lỗi', error?.message ?? 'Không tải được danh sách thợ');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      Alert.alert('Lỗi', error?.response?.data?.message || error?.message || 'Tạo thợ thất bại');
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Quản lý thợ</Text>
      ...
      <Modal visible={createVisible} transparent animationType="slide" onRequestClose={() => setCreateVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>Thêm thợ</Text>
            <TextInput
              style={styles.input}
              placeholder="Họ tên"
              value={form.fullName}
